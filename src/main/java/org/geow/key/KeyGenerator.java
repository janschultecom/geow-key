...
public class KeyGenerator implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4664364152297183528L;
	 ...
	 * @param precision
	 */
	public KeyGenerator(PRECISION precision) {
	...
	public long calculateMaxNumber(int workingPrecision) {
		...
		logger.trace("Maximum number {}.", maxNumber);
		return myMaxNumber;
	}
	...
	public long encodeSequential(final double longitude, final double latitude) {
		logger.trace("Encoding pair ({},{}) with precision {}.", longitude,
				latitude, precisionBits);
		...
		logger.trace("Sequential hash {} with precision {} created.", hash,
				precisionBits);
		return hash;
	...
	public long encodeParallel(final double longitude, final double latitude) {
		...
		logger.trace("Parallel hash {} with precision {} created .", hash,
				precisionBits);
		return hash;
	...
	public double[] decodeSequential(long hash) {
		logger.trace("Decoding sequential hash {} with precision {}.", hash,
				precisionBits);
		double longitude;
		double latitude;
		double[] longitudeInterval = { -180.0, 180.0 };
		...
		logger.trace("Sequential hash {} decoded to ({},{}).", hash, longitude,
				latitude);
	...
	public double[] decodeParallel(long hash) {
		logger.trace("Decoding parallel hash {} with precision {}.", hash,
				precisionBits);
		double longitude;
		double latitude;
		double[] longitudeInterval = { -180.0, 180.0 };
		...
		longitude = (longitudeInterval[0] + longitudeInterval[1]) / 2D;
		latitude = (latitudeInterval[0] + latitudeInterval[1]) / 2D;
		logger.trace("Parallel hash {} decoded to ({},{}).", hash, longitude,
				latitude);
		return new double[] { longitude, latitude };
	}
	...
	public long[] splitHash(long hash) {
		logger.trace("Separating parallel hash {}.", hash);
		...
		logger.trace("Parallel hash split up into {} and {}.", longitudeHash,
				latitudeHash);
	...
	public long joinHash(long longitudeHash, long latitudeHash) {
		logger.trace("Joining longitude hash {} with latitude hash {}.",
				longitudeHash, latitudeHash);
		long joinedHash = 0L;
		...
		logger.trace("Joined parrallel hash {}.", joinedHash);
		return joinedHash;
	}
	...
	public Long getGeoID(Long hash, Long id) throws IndexOutOfBoundsException {
		logger.trace("Creating geoId for hash {} and id {}.", hash, id);
		Long geoID = 0L;
		geoID |= hash;
		if (id > maxNumber) {
		...
		geoID |= hash;
		geoID |= id;
		logger.trace("GeoId {} created",geoID);
		return geoID;
	}
	...
	public Long[] getGeoIdRange(Long hash) {
		logger.trace("Creating geoId range for hash {}.",hash);
		Long[] range = new Long[2];
		range[0] = getHash(hash);
		// range[1] = getHash(hash) | maxNumber;
		range[1] = hash | maxNumber;
		logger.trace("Range ({},{}) created.",range[0],range[1]);
		return range;
	}
